...
//   - i default qui sotto DEVONO restare identici alle istruzioni storiche: la suite di
//     regressione li esercita via fire() e' la rete di sicurezza contro le divergenze.
//
...
export const DEFAULT_PROMPTS = {
  ...
  // --- fase plan ---
  'plan-write': `FASE: plan. Manca .omc-loop/plan.md. PRIMA esplora il codice rilevante (moduli coinvolti, pattern esistenti, test attuali), POI scrivi il piano come checklist markdown ('- [ ] step'), step piccoli e verificabili.{{extPlanHint}} Poi valuta la complessita' del task e registrala con: {{LOOP}} complexity low|medium|high (instrada i modelli delle fasi successive). Infine fermati.`,
  ...
  'implement-first': `FASE: implement. Apri .omc-loop/plan.md e implementa il PRIMO step non spuntato.{{implHint}} NON spuntare la casella ora: si spunta solo dopo che la review e' passata. Se per procedere serve input dell'utente: esegui {{LOOP}} pause e poi fai la domanda.`,
