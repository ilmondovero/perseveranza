...
import { askProvider, providerModels } from '../../providers/registry.mjs';
...
export async function run({ argv, rawArgv, cwd, env }) {
  ...
  for (const m of models) {
    console.log(`Asking ${provider}${m ? ` / ${m}` : ''} (slot: ${slot})...`);
    ...
    const file = join(paths.gateDir, `external-${slot}-${fileSafe(provider)}${m ? `-${fileSafe(m)}` : ''}.md`);
    ...
    appendJournal(paths.gateDir, { type: 'ask', provider, model: m || null, slot, ok: r.ok, chars: String(r.output).length });
    console.log(`\n----- ${label} -----\n${r.output}\n`);
  }
  return anyOk ? 0 : 1;
}
