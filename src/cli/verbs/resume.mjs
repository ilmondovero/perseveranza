...
export function run({ argv, cwd, env = process.env }) {
  ...
  if (takeover) {
    ...
    if (s.owner.sessionId) {
      console.log(`  owner session ${s.owner.sessionId.slice(0, 8)}, last fire ${describeLastFire(s, Date.now(), staleMs)}. If that session is gone, resume --takeover hands the loop to the session that runs it.`);
    }
  }
  return 0;
}
