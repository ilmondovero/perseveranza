...
import { describeLastFire, openStepTitles, DEFAULT_STALE_MS } from '../../core/staleness.mjs';
...
export function recap(state, planText, { now = Date.now(), staleMs = DEFAULT_STALE_MS } = {}) {
  if (!state) return [];
  ...
  lines.push(`  session:     ${state.owner?.sessionId ? state.owner.sessionId.slice(0, 8) : 'not claimed'}, last fire ${describeLastFire(state, now, staleMs)}`);
  return lines;
}
...
export function run({ argv, cwd, env }) {
  ...
  for (const l of recap(state, planText, { staleMs: parseTimeoutMs(env?.OMC_LOOP_STALE_MS, DEFAULT_STALE_MS) })) console.log(l);
