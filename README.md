...
Ciclo autonomo a feedback per Claude Code (Windows): dato un task, Claude pianifica,
implementa step per step, fa revisionare ogni step da un subagent e chiude solo dopo
una verifica finale avversariale indipendente. Una notifica avvisa quando il progetto
e' finito o quando serve intervento umano.
...
comando `/perseveranza`, quindi non interferisce con le chat normali.

## Requisiti

- Windows con PowerShell (5.1 o 7+) e [Claude Code](https://claude.com/claude-code)
...
- Opzionale: modulo PowerShell **BurntToast** per le notifiche toast
  (`Install-Module BurntToast`); senza, il fallback e' un beep
...
Lo script copia i file in `~\.claude\` e registra lo Stop hook in
`~\.claude\settings.json` (idempotente: rilanciarlo non duplica nulla; prima di
modificare `settings.json` ne crea un backup). Riavviare Claude Code dopo l'installazione.
...
`hooks\omc-loop.ps1`. Lo storico delle transizioni e' in `.omc-loop\history.log`.
...
Rimuovere `hooks\omc-loop.ps1`, `hooks\loop-drive.ps1` e `commands\perseveranza.md`
da `~\.claude\`, e togliere da `~\.claude\settings.json` la voce `hooks.Stop` il cui
`command` contiene `loop-drive.ps1`.
